...
function mappedFields(event){const fields=document.createElement('div');fields.className='field-map';fields.append(fieldChip('eventType',event.eventType,'meta'),fieldChip('frame',event.frame||'domain-only','meta'));for(const[name,value]of Object.entries(event.frameElements||{})){const implicit=value?.implicit||!value?.text,sameAsPenalty=name==='Response_action'&&value?.text===event.penaltyCode?.text,type=sameAsPenalty?'code':fieldType(name);fields.append(fieldChip(name,implicit?'implicit':value.text,type,implicit))}fields.append(fieldChip('polarity',event.polarity,'meta'),fieldChip('modality',event.modality,'meta'));return fields}
