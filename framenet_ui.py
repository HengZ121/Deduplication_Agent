...
function mappedFields(event){const fields=document.createElement('div');fields.className='field-map';fields.append(fieldChip('eventType',event.eventType,'meta'),fieldChip('frame',event.frame||'domain-only','meta'),fieldChip('trigger',event.trigger,'trigger'));for(const[name,value]of Object.entries(event.frameElements||{})){const implicit=value?.implicit||!value?.text,sameAsPenalty=name==='Response_action'&&value?.text===event.penaltyCode?.text,type=sameAsPenalty?'code':fieldType(name);fields.append(fieldChip(name,implicit?'implicit':value.text,type,implicit))}fields.append(fieldChip('polarity',event.polarity,'meta'),fieldChip('modality',event.modality,'meta'),fieldChip('source',`sentence ${event.source.sentence_index}`,'meta'));return fields}
