...
function cleanMapping(data){return{schemaVersion:data.schemaVersion,sourceDocument:data.sourceDocument,annotationMethod:data.annotationMethod,eventCount:data.eventCount,events:data.events.map(event=>({eventType:event.eventType,frame:event.frame,trigger:event.trigger,frameElements:event.frameElements,ruleCondition:event.ruleCondition,penaltyCode:event.penaltyCode,polarity:event.polarity,modality:event.modality,source:event.source})),warnings:data.warnings}}
...
function mappedFields(event){const fields=document.createElement('div');fields.className='field-map';fields.append(fieldChip('eventType',event.eventType,'meta'),fieldChip('frame',event.frame||'domain-only','meta'),fieldChip('trigger',event.trigger,'trigger'));for(const[name,value]of Object.entries(event.frameElements||{})){const implicit=value?.implicit||!value?.text;fields.append(fieldChip(name,implicit?'implicit':value.text,fieldType(name),implicit))}if(event.ruleCondition?.text)fields.append(fieldChip('ruleCondition',event.ruleCondition.text,'reason'));if(event.penaltyCode){const codeText=event.penaltyCode.text||event.penaltyCode.code;const grounded=event.source.sentence.toLowerCase().includes(String(codeText).toLowerCase())||event.source.sentence.toLowerCase().includes(String(event.penaltyCode.code).toLowerCase());fields.append(fieldChip('penaltyCode',grounded?codeText:`inherited ${event.penaltyCode.code}`,'code',!grounded))}fields.append(fieldChip('polarity',event.polarity,'meta'),fieldChip('modality',event.modality,'meta'),fieldChip('source',`sentence ${event.source.sentence_index}`,'meta'));return fields}
